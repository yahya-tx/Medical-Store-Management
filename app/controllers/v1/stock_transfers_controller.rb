class V1::StockTransfersController < ApplicationController
  ...
  def index
    ...
    if current_user.super_admin? && params[:branch_id]
      @branch = Branch.find(params[:branch_id])
      @stock_transfers = @branch.stock_transfers.where.not(pdf: nil).where(approved_by_id: current_user.id).page(params[:page]).per(@per_page)
    else
  ...
  def upload_pdf
    if @stock_transfer.update(stock_transfer_params)
      ...
      redirect_to v1_branch_stock_transfer_path(@branch, @stock_transfer)
    else
  ...
  def create
    ...
    if handle_medicines(params[:stock_transfer][:medicine_name], params[:stock_transfer][:quantity])
      if @stock_transfer.save
        ...
        redirect_to  v1_branch_stock_transfers_path(@branch)
      end
    else
      flash[:alert] = "Invalid Quantity or Medicine not available for Stock Transfer"
      redirect_to  v1_branch_stock_transfers_path(@branch)
  ...
  def approve
    ...
    redirect_to v1_branch_stock_transfer_path(stock_transfer.branch_id, stock_transfer)
  end
